...
SerialPort::SerialPort(const std::string &device_file, int baudrate)
  ...
  this->file_descriptor =
      open(device_file.c_str(), O_RDWR | O_NDELAY | O_NONBLOCK);
  if (this->file_descriptor < 0) {
    throw std::runtime_error("SERIAL: Failed to open serial device: " +
                             this->device_file);
  }
...
std::string SerialPort::read(size_t max_length) {
  ...
  if (bytes_read < 0) {
    throw std::runtime_error("SERIAL: Reading from serial device failed");
  }
